...
- Ordinary single-line parameter values keep up to 70 Unicode code points. Multiline-capable `command`, `error`, and `result` fields render after ordinary fields even when their current value is single-line, preserve their complete sanitized content whenever the whole status fits, and retain producer-supplied line breaks without adding hard wraps; visual wrapping of long source lines is left to the Discord client. `active-memory` result text follows this rule. Within `skill-harness`, `topic`, `basis`, `reason`, and `result` use the same behavior and render in that explicit order. Only when the complete status exceeds its whole-message bound does the renderer precisely truncate the lowest-priority eligible fields, append exact `(+N chars)` (`char` when singular) counts, and avoid splitting surrogate pairs.
...
- Internal `active-memory` and `skill-harness` headers show a non-bold light-gray disclosure marker matching the `(+N chars)` omission hints: `▾` while details are expanded and `▸` after global bounding collapses the group. Before semantic removal or atomic group collapse, bounded rendering tries precise value truncation within the lowest active retention-priority tier. Collapsed group details do not contribute to the omission marker. Remaining ordinary parameters roll out before results, errors, nested tool headers, and top-level status headers; equal-priority details roll out oldest first. Compact scalar rows and multiline values remain atomic, and tree connectors are recomputed after removal.
