...
- Internal `active-memory` and `skill-harness` headers show a non-bold light-gray disclosure marker matching the `(+N chars)` omission hints: `▾` while details are expanded and `▸` after global bounding collapses the group. When the whole status is too long, bounded rendering first collapses `active-memory`, then `skill-harness`; if that is insufficient, it removes the collapsed `active-memory` header before the collapsed `skill-harness` header. Collapsed descendants do not contribute to the omission baseline, while each subsequently removed collapsed header contributes one omitted line. Only then does precise multiline truncation preserve as much newer content as possible within the lowest active retention-priority tier. Remaining ordinary parameters roll out before results, errors, nested tool headers, and top-level status headers; equal-priority details roll out oldest first. Compact scalar rows and multiline values remain atomic, and tree connectors are recomputed after removal.
