...
- Internal-group display state is generation-scoped and monotonic: `expanded → collapsed → removed`. Confirmed Discord creates/edits advance `confirmedDisplayState`; exhausted network or final `5xx` outcomes advance `monotonicSafetyFloor`; explicit rejection advances neither. Render from their per-group maximum, and reset both on replacement or cleanup.
...
- Global bounding operates on semantic nodes before tree connectors are rendered. Apply prior display state, outer entry limits, normal slicing, and per-group child limits before selecting the newest visible multiline owner. When that owner belongs to an internal group, collapse the other complete internal group first and transactionally try older details whose retention priority is no higher than the owner; rollback a failed preservation attempt before collapsing the owner. Error retention priority overrides recency. Only after complete internal groups collapse may their headers be removed. Rebuild the omission baseline without collapsed descendants or completely removed blocks. A collapsed failed group preserves its `✘` header while it remains visible but may hide child and parent error details. For remaining blocks, remove ordinary parameters before result, child-error, parent/main-error, and nested-header nodes; among equal priorities remove the older source detail first. Compact scalar rows and multiline fields are atomic. Re-render connectors after every semantic removal. The state-aware renderer must reach a fixed point: rendering the same history with its returned state produces identical content and state.
