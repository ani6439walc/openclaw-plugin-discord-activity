...
- Global bounding operates on semantic nodes before tree connectors are rendered. When the whole status exceeds its bound, collapse the complete `active-memory` block first, then the complete `skill-harness` block. If both collapsed headers still do not fit, remove `active-memory` first and `skill-harness` second. Rebuild the omission baseline without collapsed descendants; a subsequently removed collapsed header contributes one omitted line. Only after these internal-group steps, precisely truncate eligible multiline-capable fields within the lowest active retention-priority tier, oldest first, just enough to fit; if that tier cannot make the message fit, continue with semantic removal rather than truncating a higher-priority error prematurely. A collapsed failed group preserves its `✘` header while it remains visible but may hide child and parent error details. For remaining blocks, remove ordinary parameters before result, child-error, parent/main-error, and nested-header nodes; among equal priorities remove the older source detail first. Compact scalar rows and multiline fields are atomic. Re-render connectors after every semantic removal.
