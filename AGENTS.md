...
- Global bounding operates on semantic nodes before tree connectors are rendered. Before semantic removal, precisely truncate eligible multiline-capable fields within the lowest active retention-priority tier just enough to fit the whole-message bound; if that tier cannot make the message fit, continue with semantic removal rather than truncating a higher-priority error prematurely. Internal `active-memory` and `skill-harness` headers always include a non-bold light-gray disclosure marker matching the `(+N chars)` omission hints: `▾` while expanded and `▸` when collapsed. If the next removal candidate belongs to one of these atomic blocks, collapse the complete block to its header and rebuild the omission baseline without its descendants. Multiple internal groups may collapse independently. A collapsed failed group preserves its `✘` header but may hide child and parent error details. For remaining blocks, remove ordinary parameters before result, child-error, parent/main-error, and nested-header nodes; among equal priorities remove the older source detail first. Compact scalar rows and multiline fields are atomic. Re-render connectors after every semantic removal.
